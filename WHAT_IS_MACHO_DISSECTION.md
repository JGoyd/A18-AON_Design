...
**Observation 3 — Runtime configuration surface.** A section named `_rtk_patchbay` holds 879 bytes of a tagged runtime-configuration table. Enumerated in full (see [`PATCHBAY.md`](PATCHBAY.md)): 62 entries in Apple's `GKTS` sticker format, covering per-device identity (chip ID, boot nonce, KASLR slide, PRNG seed), runtime layout, behavior flags (log level, debug enable, dev mode), IO base addresses, and a direct pointer to a companion tunables table. Forty-five of the sixty-two slots ship as zero in the image and are written by the loader at bring-up. The behavior of Rose — log verbosity, debug state, dev mode, memory layout — is not fixed at build time; it is finalized at boot by the loader.
